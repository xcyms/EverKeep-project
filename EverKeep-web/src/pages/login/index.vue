    ...
    <div class="flex-1 flex items-center justify-center flex-col">
        ...
        <a-form layout="vertical" :model="formState" @finish="handleSubmit">
          <a-form-item label="用户名" name="username">
            <a-input v-model:value="formState.username" placeholder="请输入用户名">
              <template #prefix>
                <div class="i-ant-design:user-outlined text-gray-400" />
              </template>
            </a-input>
          </a-form-item>
          ...
          <a-form-item label="密码" name="password">
            <a-input-password v-model:value="formState.password" placeholder="请输入密码">
              <template #prefix>
                <div class="i-ant-design:lock-outlined text-gray-400" />
              </template>
            </a-input-password>
          ...
          <a-button type="primary" html-type="submit" block size="large" :loading="loading" class="h-11 rounded-lg">
            {{ isLogin ? '登 录' : '注 册' }}
          </a-button>
        </a-form>
      ...
      <div class="py-4 text-center">
        <a href="https://beian.miit.gov.cn/" target="_blank" rel="noopener noreferrer"
          class="text-gray-400 text-xs hover:text-blue-500 transition-colors">
          蒙ICP备20000077号-3
        </a>
      </div>
    </div>
